...
public class PartnerEndPoint {
  ...
  public Response getById(@PathParam("id") Long id) {
    ...
    if (entity != null && entity.getId() > 0) {
      ...
      JsonObject json = new JsonObject();
      json.addProperty("status", ActionResult.FAILURE1_CODE);
      json.addProperty("message", "Partner not found.");
      return Response.serverError().entity(json.toString()).build();
    }
  }//</editor-fold>
  ...
  public Response Deletes(@ApiParam(value = "Form data", examples
      = @Example(value
      = @ExampleProperty("{\"ids\": \"1,2\"}"))) String formData) {
    int count = 0;
    JsonObject array = new Gson().fromJson(formData, JsonObject.class);
    String lstId = Utils.getAsString(array, "ids", "");
    if (lstId != null && lstId.trim().length() > 0) {
      String[] tmp = lstId.split(",");
      long id = 0L;
      for (String ele : tmp) {
        try {
          id = Long.parseLong(ele);
