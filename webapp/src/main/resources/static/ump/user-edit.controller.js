UserWebApp.controller('UserEditController', function ($scope, $rootScope, HttpService, $translate) {
  ...
  $rootScope.branchId = $('.dataResponse').attr('data-branchid');
  $rootScope.branchName = $('.dataResponse').attr('data-branchname');
  ...
  function renderDefaultRequestParams() {
    ...
    HttpService.getData('/users/get-data-search', {}).then(function (response) {
      ...
      $scope.params.branchId = $rootScope.branchId;
      $scope.params.branch = $rootScope.branchId + "~" + $rootScope.branchName;
  ...
  $scope.onUserEdit = function () {
    ...
    for (var i = 0; i < $scope.branches.length(); i++) {
      objBrach = $scope.branches[i];
      if (objBrach.id == $scope.params.branchId) {
        $scope.params.branch = objBrach.id + "~" + objBrach.name;
        break;
      }
    }
